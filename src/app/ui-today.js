...
function setRow(st,ei,si,no,bwMove){   // no = the working-set number (warm-ups don't count: W, 1, 2, 3); bwMove: weight = extra load, blank shows "BW"
  const wv=st.w!==''&&st.w!=null&&!(bwMove&&+st.w===0)?st.w:'';
  return `<div class="set-row ${st.done?'done':''}" data-si="${si}">
    ...
    <div class="numwrap"><button data-step="w" data-d="-1" data-ei="${ei}" data-s="${si}" aria-label="Less weight">−</button>
      <input inputmode="decimal" data-f="w" data-ei="${ei}" data-s="${si}" value="${wv}" placeholder="${bwMove?'BW':'0'}" aria-label="${bwMove?'Added weight (blank = just bodyweight)':'Weight'}">
