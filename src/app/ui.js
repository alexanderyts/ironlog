...
function bind(){
  ...
  const gp=$('#groupPick');if(gp)gp.addEventListener('click',e=>{const b=e.target.closest('[data-g]');if(!b)return;const g=b.dataset.g;draft.groups.has(g)?draft.groups.delete(g):draft.groups.add(g);b.classList.toggle('on');
    refreshBuildBtns();bindBuild();});
  bindClick('#coachNudge',()=>{const el=$('#coachNudge');if(!el)return;draft.groups=new Set(el.dataset.groups.split(','));render();});
  bindBuild();
  bindClick('#btnBlank',()=>startSession({ids:[],msg:draft.deload?'Deload — lighter loads, focus on the stretch':null,deload:draft.deload,source:'blank'}));
  bindClick('#deloadToggle',()=>{draft.deload=!draft.deload;const b=$('#deloadToggle');if(b)b.classList.toggle('on',draft.deload);});
