...
test('press-heavy, leg-less history triggers balance + gap warnings',()=>{
  const now=Date.now();
  const hist=[
  ...
  assert.equal(a.push,10);assert.equal(a.pull,0);assert.equal(a.lowerSets,0);
...
test('healthy balance is reported as good',()=>{
  const now=Date.now();
  const hist=[session(1,[['barbell-bench-press',[set(135,8),set(135,8),set(135,8)]],['barbell-row',[set(135,8),set(135,8),set(135,8)]]],{now})];
